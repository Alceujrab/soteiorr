    ...
    <div class="glass-card rounded-2xl p-6 sm:p-8 space-y-6">
        ...
        <form action="{{ route('admin.raffles.update', $raffle->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="space-y-1.5">
                ...
                <input type="number" name="total_numbers" required min="10" max="1000000" value="{{ old('total_numbers', $raffle->total_numbers) }}" placeholder="Ex: 200000" class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-slate-700">
